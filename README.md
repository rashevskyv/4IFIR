...
1. [Как использовать 4IFIR](#как-использовать-4ifir)
   ...
   * [Разгон](#разгон)
      * [Настройки частот и говернор (governor) (4IFIR RatTest)](#настройки-частот-и-говернор-governor-4ifir-C-Virus)
...
В данный момент на некоторых моделях свитчей на версии 4IFIR 1.9 имеется проблема со сном, прошу обратиться в [чат помощи](https://t.me/kef4ir/205364)(имеется костыль, решающий проблему, автор проекта о проблеме знает, ведется работа)
...
1. Запустите фулл ассист (для этого откройте меню uberhand (L+R+UP), затем нажмите вправо и откройте Wizard (волшебник). Потом откройте Full Assist и приступайте к настройке.

2. Вас попросят выбрать один из 7-ми режимов, выбирайте сначала самый крутой (супер) \ или самый энергоэффективный (ECO) режим и пробуйте пройти мемтест и поиграть в игры. Можно также оставить приставку поваляться в режиме сна часов 7.  Иногда может не включиться. Если всё ок, оставляем. Если какие-то ошибки, то выберите более слабую\энергозатратную конфигурацию.

3. Далее вас будут аналогично спрашивать различные настройки. Чем круче цифра - тем лучше, но нестабильнее. Подробнее узнать о них можно, нажав Y. Проверяйте их работоспособнось аналогично. Обращать внимание стоит на заголовок, есть три варианта:
   - в заголовке содержится CPU
   - в заголовке содержится GPU
   - в заголовке содержится MEM\Timing
Разгоняем "в щи" (в Maximized) соответствующую настройку.

В первом случае стоит разгонять процессор (CPU) и проходить memtest, а затем тестировать в процессоро-зависимых играх (shifu\doom\anno mutationem)

Во втором случае тестируем GPU в GPU-зависимых играх вроде Hw-Legacy. Разгонять следует сначала GPU, а затем CPU до тех пор, пока fps не перестанет повышаться (следует выбрать локацию, где он так и не достигает 60) - это будет означать максимальную нагрузку на GPU.

В третьем случае в Maximized ставим оперативную память (MEM), и снова сначала в memtest, а потом в Память-чуствительные игры вроде  Zelda Totk (следует сделать что-то активное, например сразиться с крупным боссом, используя много объектов)
(названия игр носят рекомендательный характер, тестируйте тем, во что играете, это наиболее удобно)

Некоторых устроит и просто разогнать всё сразу. У меня нет мнения на этот счёт, действуйте в соответствии с вашей интуицией и знаниями описанными в предыдущих разделах 4IFIR Wizard.

4. Когда пункты закончатся, поиграйте денёк и убедитесь что всё ок. Затем, вы можете сделать бэкап в Wizard-е и сохранить полученную конфигурацию и начать прохождение FullAssist заново в надежде получить что-то более экономное\мощное (ECO\SPORT), походить по "тонкому льду". Показателем успеха являются membench и status monitor (количество звёздочек в играх).

NOTICE: если что-то пошло не так и система не загружается, зайдите в Hekate и выберите режим semi-stock. Там вы сможете зайти в Wizard и изменить значение, которое привело к вылету.
