...
>4IFIR Miracle -> Temporary overrides (после установки разгона в этом пункте, он будет работать до перезагрузки консоли) -> CPU  
...
* pMeh 21 - корректировка значения GPU vMin для 4ifir Optimised (Чем выше значение, тем ниже минимальный вольтаж, для мониторинга значений используйте InfoNX)

### 4ifir Optimised E
**Скорее всего, потеряло актуальность, так как настройки этого режима - теперь находятся в меню Gamechanger** 

Настройки для режима 4IFIR Miracle>Edit app profile/Temporary overrides/Global profile>Memory>4IFIR Optimized E  
Тестировать так же как и ранее  
* pMeh 19 - GPU vMin для 4ifir Optimised E (Чем выше значение, тем ниже минимальный вольтаж, для мониторинга значений используйте InfoNX)
* pMeh 20 - VDDQ/VDD2 для 4ifir Optimised E (Чем выше значение, тем выше вольтаж, для мониторинга значений используйте InfoNX)
